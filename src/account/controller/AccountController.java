...
//			Registration reg = new Registration(); //removed FCC
            Facade facade = new Facade(); //added FCC
            ...
            if ( facade.doRegistration( fName, lName, email, pass ) ) //changed FCC
        ...
        if ( parameters.containsKey( "login" ) )
        {
            ...
            { // changed FCC
                ...
                String[] row = Facade.getUserInfo( username ); // changed FCC

                // get the whole list of comments
                ArrayList<Comments> mylist = Facade.getComments(); // changed FCC
...
//			PasswordReset reset = new PasswordReset(); // removed FCC
            Facade facade = new Facade(); // added FCC

            if ( facade.checkEmail( email ) ) // changed FCC
                response.sendRedirect( "welcome.jsp" );
            else
            {
                request.setAttribute( "message",
        ...
        if ( parameters.containsKey( "submitAdmin" ) )
            ...
            try
            {
                ...
                Facade.makeAdmin( users ); // changed FCC
            ...
            catch ( Exception e )
            {
                ...
                // get the whole list of comments
                ArrayList<Comments> mylist = Facade.getComments(); // changed
                int total_comments = mylist.size();
            ...
            HttpServletResponse response ) throws ServletException, IOException
    {
        ...
        if ( parameters.containsKey( "requestSubmit" ) )
        {
            ...
            Facade.makeAdmin( approved ); // changed FCC
            Facade.dropFromTable( approved ); // changed FCC
            Facade.dropFromTable( denied ); // changed FCC
        ...
        else
        {
            if ( request.getParameter( "type" ).equalsIgnoreCase( "adminRequest" ) )
            {
                ...
                String result = Facade.sendRequest( username, fname,
                        lname ); // changed FCC
            ...
            if ( request.getParameter( "type" ).equalsIgnoreCase( "link" ) )
            {
                ...
                int result = Facade.getMemberID( session.getAttribute( "fname" ).toString(),
                        session.getAttribute( "lname" ).toString(), date ); // changed FCC
